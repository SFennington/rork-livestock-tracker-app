...
export default function AnalyticsScreen() {
  ...
  const analytics = useMemo(() => {
    ...
    // Get sold and donated from income records (quantity is in DOZENS, convert to eggs)
    income.forEach(record => {
      if (record.type === 'eggs' && record.quantity) {
        if (record.amount === 0) {
          totalDonated += record.quantity * 12; // Convert dozens to eggs
        } else {
          totalSold += record.quantity * 12; // Convert dozens to eggs
        }
      }
    });
    ...
    if (dailyEggHistory.length > 0) {
      dailyROIHistory = dailyEggHistory.map(day => {
        ...
        // Eggs sold/donated up to this date (quantity is in DOZENS, convert to eggs)
