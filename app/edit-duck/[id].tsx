...
export default function EditDuckScreen() {
  const { updateDuck, ducks } = useLivestock();
  ...
  useEffect(() => {
    const duck = ducks.find(c => c.id === id);
    if (duck) {
      setName(duck.name);
      setBreed(duck.breed);
      setDateAcquired(duck.dateAcquired);
      setCost(duck.cost.toString());
      setQuantity(duck.quantity?.toString() || "1");
      setColor(duck.color || "");
      setNotes(duck.notes || "");
    }
  }, [id, ducks]);
      ...
      <View style={styles.form}>
        ...
        <View style={styles.buttons}>
          ...
          <TouchableOpacity style={[styles.saveButton, { backgroundColor: colors.accent }]} onPress={handleSave}>
            <Text style={styles.saveButtonText}>Update Duck</Text>
          </TouchableOpacity>
        </View>
        </View>
      </ScrollView>
